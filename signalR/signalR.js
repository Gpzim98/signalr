...
var url = require('url'),
    ...
    WebSocketClient = require('ti-websocket-client').WebSocket;
...
function negotiateProxies(baseUrl, hubNames, onSuccess, onError, _client) {
	...
	var negotiateFunction = function(res) {
		try {
			if (res.status == 200) {
				Log.writeLog(res.responseText);
				var negotiateObj = JSON.parse(res.responseText);
				...
				if (_client.serviceHandlers.onUnauthorized) {
					...
					onError('Negotiate Unauthorized', undefined, res.status);
				}
			} else {
				onError('Negotiate Unknown', undefined, res.status);
			}
		} catch (e) {
	...
	if (negotiateUrlOptions.protocol === 'http:' || negotiateUrlOptions.protocol === 'https:') {
		Log.writeLog(negotiateUrlOptions.format());
		var negotiateResult = directinstall.httpClient.get(negotiateUrlOptions.format(), negotiateFunction, negotiateErrorFunction);
	} else if (negotiateUrlOptions.protocol === 'wss:' || negotiateUrlOptions.protocol === 'ws:') {
...
function clientInterface(baseUrl, hubs, reconnectTimeout, doNotStart) {
	...
	client.end = function() {
		if ((_client.connection.state == states.connection.connecting || _client.connection.state == states.connection.connected) && _client.websocket.connection) {
	...
	function abort() {
		...
		if (_client.connection.state === states.connection.disconnected || _client.connection.state === states.connection.disconnecting) {
			...
			if (abortUrlOptions.protocol === 'wss:') {
				abortUrlOptions.protocol = 'https:';
			} else if (abortUrlOptions.protocol !== 'http:') {
			...
			var abortCallback = function(res) {
			...
			var req = directinstall.httpClient.post(abortUrlOptions.format, null, abortCallback, abortError);
		}
	...
	function startCommunication(onSuccess, onError) {
		...
		var startFunction = function(res) {
			try {
				if (res.statusCode == 200) {
					var startObj = JSON.parse(res.responseText);
					onSuccess(startObj);
				} else if (res.statusCode == 401 || res.status == 302) {
					if (_client.serviceHandlers.onUnauthorized) {
						_client.serviceHandlers.onUnauthorized(res);
					} else {
						console.log('start::Unauthorized (' + res.status + ')');
					}
				} else {
					console.log('start::unknown (' + res.status + ')');
				}
			} catch (e) {
		...
		if (startUrlOptions.protocol === 'http:') {
			var startResult = directinstall.httpClient.get(startUrlOptions.format(), startFunction, startErrorFunction);
	...
	_client.start = function(tryOnceAgain) {
		...
		//retryingConnection: 8,
		//connecting: 2,
		if (_client.connection.state == states.connection.connected || _client.connection.state == states.connection.retryingConnection || _client.connection.state == states.connection.connecting) {
			return true;
		}
		//unbound: 0,
		//bindingError: 9,
		else if (_client.connection.state == states.connection.bindingError || _client.connection.state == states.connection.unbound) {
			if (!tryOnceAgain) {
				_client.getBinding();
		...
		//connectFailed: 6,
		//errorOccured: 7,
		else if (_client.connection.state == states.connection.bound || _client.connection.state == states.connection.disconnecting || _client.connection.state == states.connection.disconnected || _client.connection.state == states.connection.connectFailed || _client.connection.state == states.connection.errorOccured) {
	...
	_client.websocket.client.on('connectFailed', function(error) {
		if (_client.connection.state == states.connection.retryingConnection && scheduleReconnection(false)) {
		} else {
